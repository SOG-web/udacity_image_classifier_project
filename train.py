...
def train_model(args, train_data_loader, valid_data_loader, class_to_idx):
    ...
    for e in range(args.epochs):
        ...
        for images, labels in train_data_loader:
            ...
            if step % print_every == 0 or step == 1 or step == len(train_data_loader):
                print("Epoch: {}/{} Batch % Complete: {:.2f}%".format(e +
                      1, args.epochs, (step)*100/len(train_data_loader)))
    ...
    print("model saved to {}".format(os.path.join(
        args.save_directory, "checkpoint.pth")))
    return True
...
if __name__ == '__main__':
    ...
    parser.add_argument('--learning_rate', dest='learning_rate',
                        help="This is the learning rate when training the model. Default is 0.003. Expect float type", default=0.003, type=float)
    parser.add_argument('--epochs', dest='epochs',
                        help="This is the number of epochs when training the model. Default is 5. Expect int type", default=3, type=int)
    parser.add_argument(
        '--gpu', dest='gpu', help="Include this argument if you want to train the model on the GPU via CUDA", action='store_true')
    ...
    train_data_loader, valid_data_loader, class_to_idx = data_transformation(
        args)
